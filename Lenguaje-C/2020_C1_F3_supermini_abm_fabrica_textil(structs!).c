...
//a- Crear una función para la registración de rollos de tela a una fábrica textil, cada rollo 
//posee un nro de rollo y un peso en kg.
//Se pide:
...
//b- Sabiendo que la encimadora de tela utiliza rollos mayores a 12kg, crear una función que 
//sea llamada por la del pto a, que valide el peso del rollo y en caso que el mismo sea 
//menor a 12kg imprima un mensaje “Error fuera de norma”
...
int ingresarDatos(rollo_t rollos[]) {
    ...
    do {
        ...
        */
        ...
        //fgets(ingreso, MAX_STRING_SIZE + 1, stdin); //cargo el array ingreso

        printf("\n¿Desea seguir ingresando datos <1-s/0-n>?: ");
        scanf("%i",&cortar);

        //ingreso[strlen(ingreso) - 1] = '\0';    //agrego \0 xa crear el str

    } while (cortar != 0 ); //while(strcmp("n", ingreso) != 0); //if strcmp return = 0, indicates str1 is equal to str2.
                                        //mientras no sean ='s... continuar
    return cantRollos;
}
...
int rolloMayorPeso(rollo_t rollos[MAX_ROLLS], int cantRollos) {
    ...
    for(size_t i = 0; i < cantRollos; i++) {
        //printf("%lf",rollos[i].peso);
        //printf("%i  ",rollos[i].nro);
