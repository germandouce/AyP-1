// El ingeniero Guido desea realizar un análisis estratégico del juego "Among us" y para el mismo requiere realizar un
// programa en el
...
void ingresarDatos(jugador_t *jugador){
    ...
    scanf("%i",&jugador->cantVecesImpostor );
    ...
    for(size_t i = 0; i < jugador->cantVecesImpostor; i++){
        ...
        scanf("%i", &jugador->cantAsesinatos[i]);
    }

    printf("\nIngrese la cantidad de veces que fue tripulante: ");
    scanf("%i", &jugador->cantVecesTripulante);

    printf("\nIngrese la cantidad de veces que murió en total (Siendo inocente o impostor): ");
    scanf("%i", &jugador->cantMuertes);
}

int calcularTotalAsesinatos(int cantAsesinatos[], int cantVecesImpostor){  //*cantAsesinatos
    int totalAsesinatos = 0;

    for (size_t i = 0; i <cantVecesImpostor ; ++i) {
        totalAsesinatos += cantAsesinatos[i];   //totalAsesinatos += *(cantAsesinatos + i);
    }
    return totalAsesinatos;
}

void imprimirCalculo(int totalAsesinados, jugador_t jugador){
    double calculo = totalAsesinados/ (double)(jugador.cantVecesTripulante - jugador.cantMuertes );
